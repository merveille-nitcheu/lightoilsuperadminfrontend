...
export class ShowTankComponent {
    ...
    ngOnInit(){
        ...
        this.tankService.showTank(tankId).subscribe((data) => {
            ...
            this.data_temp = this.tankData.correction_data.data_temp.split(';').filter(value => value.trim() !== '')
            this.data_level = this.tankData.correction_data.data_level.split(';').filter(value => value.trim() !== '')
            this.data_pressure = this.tankData.correction_data.data_pressure.split(';').filter(value => value.trim() !== '')
    ...
    onUpdateForm(tankId: number) {
        this.loading = true;
        this.tankService
            .updateTank(tankId, this.tankForm.value)
            .subscribe(
                (response) => {
                    this.tankForm.reset();
                ...
                (error) => {
                    console.error(
                        'Erreur lors de la mise à jour du tank',
                        error
                    );
                }
            );
